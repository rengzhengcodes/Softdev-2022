...
def connect(DB_FILE:str = "storage.db"): # initiates connection to a file
	global db # sets this as a global variable
	db = sqlite3.connect(DB_FILE) #open if file exists, otherwise create
	global c # sets this as a global variable
	c = db.cursor()               #facilitate db ops -- you will use cursor to trigger db events

if __name__ == "__main__":# for testing when the module is run alone
	connect("discobandit.db")
...
def exit_db(): #closes db if its imported
	global db #referemces tje global db variable
